...
import { db } from "@/db";
import { posts, postContributors } from "@/db/schema";
import {
	PostCategory,
	PostType,
	SubCategory,
...
export async function GET(request: NextRequest) {
	try {
		...
		const postConditions = [
			// Posts where user is author OR contributor
			or(
				eq(posts.authorId, currentUser.id),
				// Subquery to check if user is a contributor
				inArray(
					posts.id,
					db
						.select({ postId: postContributors.postId })
						.from(postContributors)
						.where(eq(postContributors.userId, currentUser.id)),
				),
			),
		];
