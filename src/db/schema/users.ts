...
import { userRoleEnum, accountStatusEnum, subscriptionPlanEnum } from "./enums";
...
export const userProfiles = pgTable(
	...
	(table) => ({
		userIdIdx: index("UserProfile_userId_idx").on(table.userId),
		emailIdx: index("UserProfile_email_idx").on(table.email),
	}),
);
...
export const follows = pgTable(
	...
	(table) => ({
		followerFollowedIdx: uniqueIndex("Follow_followerId_followedId_idx").on(
			table.followerId,
			table.followedId,
		),
		followerIdx: index("Follow_followerId_idx").on(table.followerId),
		followedIdx: index("Follow_followedId_idx").on(table.followedId),
	}),
);
