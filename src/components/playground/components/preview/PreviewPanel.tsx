...
// Components
const LoadingState: React.FC<LoadingStateProps> = React.memo(({ message, subMessage, showDots }) => (
...
const WelcomeState: React.FC = React.memo(() => (
	<div className="h-full bg-background flex items-center justify-center">
		<div className="text-center text-muted-foreground max-w-md">
			...
			<h3 className="text-xl font-semibold text-foreground mb-3">Welcome to Online IDE</h3>
			<p className="text-muted-foreground mb-6">
				Select a template from the dropdown above to start coding. Your changes will appear here in real-time.
			</p>
			<div className="grid grid-cols-2 gap-4 text-sm">
				{Object.entries(TEMPLATE_ICONS).map(([key, icon]) => (
					<div key={key} className="bg-card p-3 rounded-lg border">
						<div className="text-2xl mb-2">{icon}</div>
						<div className="font-medium">{TEMPLATE_NAMES[key as keyof typeof TEMPLATE_NAMES]}</div>
					</div>
				))}
			</div>
		</div>
